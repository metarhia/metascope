...
const { chrome, paint, OSC_BG_BLACK, OSC_BG_RESET } = theme;
const { OSC_POINTER, OSC_POINTER_RESET } = theme;
...
class Viewer {
  ...
  handleMouse(key) {
    ...
    if (baseBtn === 0 && key.release) {
      ...
      if (drag && this.isControlClick(drag, pos || drag.last)) {
        this.selection = null;
  ...
  stickyCopyTarget(block, width) {
    ...
    if (copyLine >= viewStart && copyLine <= viewEnd) {
      ...
      const col0 = 0;
      const col1 = width > 0 ? width : block.copy.col1;
      const sticky = false;
      const blockId = block.id;
