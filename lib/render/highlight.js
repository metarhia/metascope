...
const readTemplateInterp = (source, j) => {
  ...
  const n = source.length;
  while (j < n && depth > 0) {
    if (source[j] === SQUOTE || source[j] === '"' || source[j] === '`') {
...
jsFamily.highlightJsFamily = (source, ts = false) => {
  const out = [];
  let i = 0;
  const n = source.length;
  ...
  while (i < n) {
    ...
    if (ch === '`') {
      const r = jsFamily.highlightTemplate(source, i);
      out.push(r.text);
      i = r.end;
      continue;
    }
...
jsFamily.highlightTemplate = (source, i) => {
  const n = source.length;
  let j = i + 1;
  ...
  while (j < n) {
    ...
    if (source[j] === '$' && source[j + 1] === '{') {
      ...
      parts.push(jsFamily.highlightJsFamily(interp.expr, false));
      j = interp.end;
      if (source[j] === '}') {
        parts.push(paintCode('interpolation', '}'));
        j++;
...
const highlightJsFamily = (source, ts = false) =>
  jsFamily.highlightJsFamily(source, ts);
