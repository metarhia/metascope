...
const { asText, stripAnsi } = require('../wrap.js');
...
const LISP_LITERALS = ['nil', 't', 'true', 'false'];
...
const LISP_BOOLS = ['#t', '#f', '#true', '#false'];
...
const lispName = (style, allowOpen = false, openStyle = style) => ({
  style,
  allowOpen,
  openStyle,
});

const LISP_INTRODUCES = {
  defun: lispName('function', true),
  'defun*': lispName('function', true),
  defmacro: lispName('function', true),
  'define-syntax': lispName('function', true),
  define: lispName('variable', true, 'function'),
  defmethod: lispName('function'),
  defgeneric: lispName('function'),
  defn: lispName('function'),
  'defn-': lispName('function'),
  defmulti: lispName('function'),
  defadvice: lispName('function'),
  defvar: lispName('variable'),
  defparameter: lispName('variable'),
  defconstant: lispName('variable'),
  defconst: lispName('variable'),
  defcustom: lispName('variable'),
  def: lispName('variable'),
  defclass: lispName('className'),
  defstruct: lispName('className'),
  deftype: lispName('className'),
  defrecord: lispName('className'),
  defpackage: lispName('className'),
  ns: lispName('className'),
};
...
const TS_WORD_STYLE = Object.assign({}, WORD_STYLE, wordStyle(TYPES, 'type'));
...
const plainTail = (parts) => stripAnsi(parts.join('')).slice(-8);
...
const stylePascal = ({ word }) => (isFirstUpper(word) ? 'className' : null);
...
const isRegexContext = (parts) => {
  const prev = stripAnsi(parts.join('')).trimEnd();
  if (!prev) return true;
...
// Mutual recursion: template ↔ JS family
class JsHighlighter {
  ...
  highlightTemplate(source, i) {
    ...
    while (j < n) {
      ...
      if (source[j] === '$' && source[j + 1] === '{') {
        ...
        parts.push(this.highlightJsFamily(interp.expr, false));
        j = interp.end;
        if (source[j] === '}') {
          parts.push(paintCode('interpolation', '}'));
          j++;
...
const highlightJson = (source) => {
  ...
  while (i < n) {
    ...
    if (/[a-z]/.test(ch)) {
      ...
      const isLit = JSON_LITERALS.includes(word);
...
const bashWordStyle = (word, nextCh) => {
  const styled = BASH_WORD_STYLE[word];
...
const LISP_WORD_STYLE = Object.assign(
  {},
  wordStyle(LISP_CONTROL, 'control'),
  wordStyle(LISP_LITERALS, 'literal'),
  wordStyle(LISP_BUILTINS, 'function'),
  wordStyle(LISP_STORAGE, 'storage'),
);

const LISP_DECIMAL = /^[+-]?(?:\d+\.\d*|\.\d+|\d+)(?:[eE][+-]?\d+)?[NM]?$/;

const LISP_RATIO = /^[+-]?\d+\/\d+$/;
const LISP_RADIX = /^#[bBoOxX][0-9a-zA-Z]+$/;
const LISP_BASE = /^#\d+[rR][0-9a-zA-Z]+$/;
const LISP_INF = /^[+-](?:inf|nan)\.0$/i;
...
const isLispNumber = (word) =>
  LISP_DECIMAL.test(word) ||
  LISP_RATIO.test(word) ||
  LISP_RADIX.test(word) ||
  LISP_BASE.test(word) ||
  LISP_INF.test(word);

const isLispBool = (word) => LISP_BOOLS.includes(word.toLowerCase());

const isLispKeyword = (word) => word.startsWith(':') && word.length > 1;

const styleLispSymbol = (word) => {
  if (isLispNumber(word)) return 'number';
  if (isLispKeyword(word)) return 'constant';
  if (isLispBool(word)) return 'literal';
  if (LISP_OP_WORDS.includes(word)) return 'operator';
  if (word === '.') return 'punct';
  if (word.startsWith('.') && word.length > 1) return 'method';
  return LISP_WORD_STYLE[word.toLowerCase()] || 'variable';
};

const splitOnSep = (word, sep) => {
  const idx = word.indexOf(sep);
  if (idx <= 0) return null;
  const name = word.slice(idx + sep.length);
  if (!name) return null;
  return { pkg: word.slice(0, idx), sep, name };
};

const splitLispSymbol = (word) => {
  if (word.includes('|') || word.startsWith(':')) return null;
  if (isLispNumber(word)) return null;
  const colon = splitOnSep(word, '::') || splitOnSep(word, ':');
  if (colon) return colon;
  return splitOnSep(word, '/');
};

const lispBareName = (word) => {
  const parts = splitLispSymbol(word);
  return parts ? parts.name : word;
};

const beginLispName = (word) => {
  const intro = LISP_INTRODUCES[word.toLowerCase()];
  if (!intro) return null;
  return {
    style: intro.style,
    openStyle: intro.openStyle,
    allowOpen: intro.allowOpen,
    open: false,
  };
};

const onLispOpen = (pending) => {
  if (!pending || pending.open || !pending.allowOpen) return null;
  return {
    style: pending.openStyle,
    openStyle: pending.openStyle,
    allowOpen: false,
    open: true,
  };
};

const clearLispState = (state) => {
  state.pending = null;
  state.typeNext = false;
};

const pushStyled = (out, style, text) => {
  const parts = text.split('\n');
  for (let i = 0; i < parts.length; i++) {
    if (parts[i]) out.push(paintCode(style, parts[i]));
    if (i < parts.length - 1) out.push('\n');
  }
};

const pushLispQualified = (out, word, nameStyle) => {
  const parts = splitLispSymbol(word);
  if (!parts) {
    const style = nameStyle || styleLispSymbol(word);
    pushStyled(out, style, word);
    return;
  }
  pushStyled(out, 'type', parts.pkg);
  pushStyled(out, 'punct', parts.sep);
  const style = nameStyle || styleLispSymbol(parts.name);
  pushStyled(out, style, parts.name);
};

const emitLispSymbol = (out, word, state) => {
  if (!word) return;
  if (state.typeNext) {
    state.typeNext = false;
    pushStyled(out, 'type', word);
    return;
  }
  if (state.pending) {
    const style = state.pending.style;
    state.pending = null;
    pushLispQualified(out, word, style);
    return;
  }
  if (isLispNumber(word) || isLispBool(word) || isLispKeyword(word)) {
    pushStyled(out, styleLispSymbol(word), word);
    state.pending = null;
    return;
  }
  pushLispQualified(out, word, '');
  state.pending = beginLispName(lispBareName(word));
};

const readLispToken = (source, i) => {
  const n = source.length;
  let j = i;
  while (j < n && !isLispTerm(source[j])) {
    if (source[j] === '\\' && j + 1 < n) {
      j += 2;
      continue;
    }
...
const readLispChar = (source, i) => {
  let j = i + 2;
  const n = source.length;
  if (j >= n) return { text: source.slice(i), end: n };
  if (/[A-Za-z]/.test(source[j])) {
    while (j < n && !isLispTerm(source[j])) j++;
    return { text: source.slice(i, j), end: j };
  }
  return { text: source.slice(i, j + 1), end: j + 1 };
};

const readLispBlockComment = (source, i) => {
  let j = i + 2;
  let depth = 1;
  const n = source.length;
  while (j < n && depth > 0) {
...
const takeLispDispatch = (source, i, out, state) => {
  const next = source[i + 1] || '';
  const atBol = i === 0 || source[i - 1] === '\n';

  if (next === '|') {
    const { text, end } = readLispBlockComment(source, i);
    pushStyled(out, 'comment', text);
  ...
  if (next === '\\') {
    const { text, end } = readLispChar(source, i);
    pushStyled(out, 'literal', text);
    clearLispState(state);
    return { done: true, end };
  }

  if (next === '"') {
    const { text, end } = readLispString(source, i + 1);
    pushStyled(out, 'regex', `#${text}`);
    clearLispState(state);
    return { done: true, end };
  }

  if (next === ';' || next === '_') {
    pushStyled(out, 'comment', source.slice(i, i + 2));
    clearLispState(state);
    return { done: true, end: i + 2 };
  }

  if (`${SQUOTE}:.,`.includes(next)) {
    out.push(paintCode('operator', source.slice(i, i + 2)));
    clearLispState(state);
    return { done: true, end: i + 2 };
  }
  ...
  if (next === '(' || next === '[') {
    out.push(paintCode('punct', '#'));
    clearLispState(state);
    return { done: true, end: i + 1 };
  }

  return { done: false, end: i };
};

const highlightLisp = (source) => {
  const text = asText(source);
  const out = [];
  const state = { pending: null, typeNext: false };
  let i = 0;
  const n = text.length;
  ...
  while (i < n) {
    ...
    if (ch === ';') {
      const { text: comment, end } = readUntilNl(text, i);
      pushStyled(out, 'comment', comment);
      i = end;
      continue;
    }

    if (ch === '"') {
      const { text: lit, end } = readLispString(text, i);
      pushStyled(out, 'string', lit);
      i = end;
      clearLispState(state);
      continue;
    }
    ...
    if (ch === '#') {
      const step = takeLispDispatch(text, i, out, state);
      if (step.done) {
        i = step.end;
        continue;
      }
    ...
    if (ch === SQUOTE || ch === '`') {
      out.push(paintCode('operator', ch));
      i++;
      clearLispState(state);
      continue;
    }
    ...
    if (ch === ',') {
      const op = next === '@' ? ',@' : ',';
      out.push(paintCode('operator', op));
      i += op.length;
      clearLispState(state);
      continue;
    }
    ...
    if ('([{'.includes(ch)) {
      ...
      i++;
      state.typeNext = false;
      const opened = ch === '(' ? onLispOpen(state.pending) : null;
    ...
    const token = readLispToken(text, i);
    if (token.end === i) {
      out.push(ch);
      i++;
      continue;
    }
    emitLispSymbol(out, token.text, state);
    i = token.end;
  }
...
const highlight = (lang, source) => {
  if (lang === 'dts') return highlightTs(source);
  if (HIGHLIGHTERS[lang]) return HIGHLIGHTERS[lang](source);
  return (HIGHLIGHTERS[mapLang(lang)] || highlightTxt)(source);
};
