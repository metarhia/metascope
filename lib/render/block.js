...
const runBodyLines = (result) => {
  let raw;
  if (result.shown != null) raw = asText(result.shown);
  else if (result.text != null) raw = asText(result.text);
  else raw = [result.stdout, result.stderr].filter(Boolean).join('\n');
...
const frameRunOutput = (result, options = {}) => {
  ...
  const hasExit = !running && result.code != null;
...
const paintErrorText = (text) =>
  `\x1b[38;2;230;90;90m${text}\x1b[0m`;
