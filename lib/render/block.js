...
const frameCodeBlock = (lang, source, options = {}) => {
  ...
  const colored = highlightLines(lang, text);
  ...
  const lastSrc = Math.max(0, sourceLineCount(text) - 1);
  const blockOrigin = { first: 0, last: lastSrc };
  ...
  for (let y = 0; y < PAD_Y; y++) {
    if (y === 0 && padTo > 0) {
      ...
      pushLine(row.line, blockOrigin);
      copy = { row: 0, col0: row.col0, col1: row.col1 };
    } else {
      pushLine(bandAnsi('', BG.code, padTo), blockOrigin);
    }
  }

  const left = ' '.repeat(PAD_X);
  if (colored.length === 0) {
    pushLine(bandAnsi(left, BG.code, padTo), { first: 0, last: 0 });
  } else {
    for (let i = 0; i < colored.length; i++) {
      const origin = { first: i, last: i };
      const wrapped = wrapFn(colored[i], bodyWidth);
      for (const part of wrapped) {
        pushLine(bandAnsi(left + part, BG.code, padTo), origin);
      }
    }
  }
  ...
  return { lines, origins, play, copy, runnable, setup, lang, source: text };
};
...
const frameQuoteBlock = (texts, options = {}) => {
  ...
  const lines = [];
  const origins = [];
  const lastText = Math.max(0, texts.length - 1);
  const first = options.first ?? 0;
  const last = options.last ?? first + lastText;
  const padOrigin = { first, last };
  ...
  for (let y = 0; y < PAD_Y; y++) {
    pushLine(band('quote', ' ', padTo), padOrigin);
  }

  for (let i = 0; i < texts.length; i++) {
    const srcLine = first + i;
    const origin = { first: srcLine, last: srcLine };
    const text = asText(texts[i]);
    // Wrap text only; repeat │ on every visual row (soft-wrap continuations).
    const rows = padTo > 0 ? wrapLine(text, bodyWidth) : [text];
    for (const row of rows) {
      pushLine(band('quote', ' '.repeat(PAD_X) + prefix + row, padTo), origin);
    }
  }

  if (texts.length === 0) {
    pushLine(band('quote', ' '.repeat(PAD_X) + prefix, padTo), padOrigin);
  }
