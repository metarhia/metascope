...
const { stripAnsi, visibleWidth } = require('./wrap');
 ...
 * Screen is always pure black (terminal-independent). */
...
const BG = {
  ...
  quote: rgb(22, 32, 24),
  hr: rgb(24, 26, 34),
  // Inline `code` — former code-block tone
...
/** Open screen (black) background. */
const screenBg = () => `\x1b[${BG.screen}m`;

/** SGR reset, then keep black screen bg
 * (avoids terminal gray/white default). */
...
const paint = (style, text) => {
  ...
  return fn ? fn(String(text)) : String(text);
};
...
const CODE_FG = {
  ...
  control: [197, 134, 192],
  storage: [86, 156, 214], // #569cd6 let/const/function
  literal: [86, 156, 214], // true/false/null/this
...
const paintCode = (style, text) => {
  ...
  // Only set fg — do not force screen black
  // (bandAnsi restores block bg after \x1b[0m).
  return `\x1b[38;2;${rgb[0]};${rgb[1]};${rgb[2]}m${text}\x1b[0m`;
};

/** Inline `code` with very soft gray background. */
...
/** Soft full-width band: theme fg + weak truecolor bg on every cell. */
 ...
 */
function bandAnsi(ansiLine, bgCode, width) {
  const ESC = '\\u001b';
  const open = `\x1b[${bgCode}m`;
  let s = String(ansiLine);
  // Drop trailing resets (plain or reset+screen-black).
  const trailRe = new RegExp(`(${ESC}\\[0m(?:${ESC}\\[48;2;0;0;0m)?)+$`, 'g');
  s = s.replace(trailRe, '');
  // After any mid-line reset, restore this band's bg
  // (strip forced screen black too).
  const midRe = new RegExp(`${ESC}\\[0m(?:${ESC}\\[48;2;0;0;0m)?`, 'g');
  const body = s.replace(midRe, `\x1b[0m${open}`);
  const pad = Math.max(0, (width || 0) - visibleWidth(ansiLine));
  // Re-open band bg before pad so nested chips
  // (exit / line num) never bleed to EOL.
  const padSpaces = pad > 0 ? ' '.repeat(pad) : '';
  return `${open}${body}\x1b[0m${open}${padSpaces}\x1b[0m`;
}
