...
/** OSC 22: mouse pointer shape (CSS names: pointer = hand). */
const OSC_POINTER = '\x1b]22;pointer\x07';
const OSC_POINTER_RESET = '\x1b]22;default\x07';
...
const headingLevel = (level) => Math.min(6, Math.max(1, level));

const headingStyle = (level) => `h${headingLevel(level)}`;

const headingBg = (level) => BG[`h${headingLevel(level)}`];
