...
const { visibleWidth, padEndVisible, sliceVisible } = require('./wrap');
const { screenLine } = require('./theme');

/** Empty row at top of the console. */
...
/** Empty columns at left and right. */
const MARGIN_X = 2;
...
const innerSize = () => {
  ...
  return {
    cols,
    rows,
    innerCols: Math.max(1, cols - MARGIN_X * 2),
    innerRows: Math.max(1, rows - marginY),
  };
};

/** Fit content to inner width, then pad with left/right margins on black. */
const frameLine = (line, cols) => {
  const inner = Math.max(1, cols - MARGIN_X * 2);
  const clipped =
    visibleWidth(line) > inner ? sliceVisible(line, 0, inner) : line;
